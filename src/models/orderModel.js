...
const deliveryInfoSchema = new mongoose.Schema({
  type: { type: String, enum: ['train', 'station', 'home'], required: true },
  trainNo: { type: String },
  trainName: { type: String },
  coach: { type: String },
  seat: { type: String },
...
const totalsSchema = new mongoose.Schema({
  subtotalCents: { type: Number, required: true },
  taxCents: { type: Number, default: 0 },
  deliveryCents: { type: Number, default: 2000 },
  discountCents: { type: Number, default: 0 },
  couponCode: { type: String },
  finalCents: { type: Number, required: true }
});
...
// ===== Payment Schema =====
const paymentSchema = new mongoose.Schema({
  method: { type: String, enum: ['UPI', 'CARD', 'WALLET', 'COD'], required: true },
...
const orderSchema = new mongoose.Schema({
  ...
  status: {
    ...
    enum: [
      ...
      'RETURNED', 'FAILED_PAYMENT'
    ],
    default: 'PENDING'
  },
  trainNo: { type: String },
  station: { type: String },
  stationIndex: { type: Number },
  scheduledArrival: { type: Date },
  scheduledDepart: { type: Date },
...
// ===== Virtuals =====
orderSchema.virtual('formattedTotal').get(function() {
  return this.totals?.finalCents ? (this.totals.finalCents / 100).toFixed(2) : '0.00';
});
...
orderSchema.pre('save', function(next) {
  ...
  if (this.isModified('status')) {
    this.history.push({ status: this.status, at: now, by: this._doc.updatedBy || this.user });

    const ts = this.timestamps;
    switch (this.status) {
      case 'CONFIRMED': if (!ts.confirmedAt) ts.confirmedAt = now; break;
      case 'PREPARING': if (!ts.preparingAt) ts.preparingAt = now; break;
      case 'READY_FOR_PICKUP': if (!ts.readyForPickupAt) ts.readyForPickupAt = now; break;
      case 'OUT_FOR_DELIVERY': if (!ts.outForDeliveryAt) ts.outForDeliveryAt = now; break;
      case 'DELIVERED': if (!ts.deliveredAt) ts.deliveredAt = now; break;
      case 'CANCELLED': if (!ts.cancelledAt) ts.cancelledAt = now; break;
    }
  }
