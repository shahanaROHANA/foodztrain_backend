...
export const protect = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (decoded.userType === "delivery") {
      req.user = await Delivery.findById(decoded.id).select("-password");
      console.log('🔍 Delivery agent found:', req.user ? { _id: req.user._id, email: req.user.email, role: 'deliveryAgent' } : 'No delivery agent');
      
      if (!req.user) {
        console.log('❌ Delivery agent not found in database');
        return res.status(401).json({ message: "Delivery agent not found" });
      }
      
      // Convert to consistent format
      req.user.role = "deliveryAgent";
    } else {
      // Regular user token
